...
$status=1;
	$insert_record=mysqli_query($conn,"INSERT INTO add_movie (`movie_name`,`directer`,`release_date`,`categroy`,`language`,`you_tube_link`,`action`,`decription`,`show`,`image`,`status`)VALUES('".$movie_name."','".$directer_name."','".$release_date."','".$categroy."','".$language."','".$trailer."','".$action."','".$decription."','".$show."','".$filename."','".$status."')");
	if(!$insert_record){
	 	echo "unsuccesfull";
	}
	else
...
echo $response;
	...
	$image=$edit_old_image;
		...
		$insert_record=mysqli_query($conn, "UPDATE `add_movie` SET `movie_name` = '$edit_movie_name', `directer` = '$edit_directer_name', `categroy` = '$edit_categroy', `language` = '$edit_language',`you_tube_link` = '$trailer',`action` = '$action',`decription` = '$decription', `show` = '$edit_show', `image` = '$image' WHERE `id` = '$e_id'"); 
